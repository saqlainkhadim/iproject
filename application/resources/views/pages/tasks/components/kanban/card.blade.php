...
<div class="kanban-card kanban-board-element show-modal-button reset-card-modal-form js-ajax-ux-request" data-toggle="modal"
    ...
    <div class="x-title wordwrap" id="kanban_task_title_{{ $task->task_id }}">{{ $task->task_title }}
        ...
        <div class="dropdown-menu dropdown-menu-small dropdown-menu-right js-stop-propagation"
            ...
            <a class="dropdown-item confirm-action-danger js-stop-propagation"
                ...
                data-url="{{ url('/') }}/tasks/timer/{{ $task->task_id }}/stop" style="display: none;">{{ cleanLang(__('lang.stop_my_timer')) }}</a>
            @php $count_actions ++ ; @endphp
            @endif
            <!--stop all timers-->
            ...
            <a class="dropdown-item confirm-action-danger js-stop-propagation"
                ...
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="GET"
                data-url="{{ url('/') }}/tasks/timer/{{ $task->task_id }}/stopall?source=list" style="display: none;">{{ cleanLang(__('lang.stop_all_timers')) }}</a>
            @php $count_actions ++ ; @endphp
            @endif
            ...
            <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                ...
                data-action-ajax-loading-target="tasks-td-container" style="display: none;">{{ cleanLang(__('lang.recurring_settings')) }}</a>
            <!--stop recurring -->
            ...
            <a class="dropdown-item confirm-action-info" href="javascript:void(0)"
                ...
                data-url="{{ urlResource('/tasks/'.$task->task_id.'/stop-recurring?source=list') }}" style="display: none;">
            ...
            <a style="display: none;" class="dropdown-item confirm-action-info {{ runtimeActivateOrAchive('archive-button', $task->task_active_state) }} card_archive_button_{{ $task->task_id }}"
    ...
    <div class="x-meta">
        ...
        <span><strong>Date Started:</strong>: {{ runtimeDate($task->task_date_start) }}</span>
        @endif
        <!--due date-->
